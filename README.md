...
3. **Robust Asset Architecture**:
   - Every product in the catalog has optimized local full-device assets in `public/products/`: a primary shot (`<slug>.jpg`) and a secondary alternate full-device shot (`<slug>-alt.jpg`), ensuring recognizable full-device framing across ProductCard, PDP hero, and thumbnail gallery.
